...
void view_vector(const std::vector<std::string>& tasklist, std::string* ptr_task)
{
	clear_screen();
	...
	do
	{
		...
		switch(choice)
		{
			case 'w':
				...
				break;
			case 's':
				if( (index+1) > tasklist.size()-1) break;
				index++;
				highlighted_task++;
				break;
			case 'd':
...
int main()
{
    std::vector<std::string> string_list = {"Hello World", "Sup homie", "Damn it Bobby!"};
